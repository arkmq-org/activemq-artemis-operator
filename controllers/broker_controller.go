...
// BrokerReconciler reconciles a Broker object (arkmq.org/v1)
...
func (r *BrokerReconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	err := r.Get(ctx, request.NamespacedName, broker)
	if err != nil {
	...
	customResource, err := ConvertBrokerToArtemis(broker)
	if err != nil {
		reqLogger.Error(err, "failed to convert Broker to internal representation")
		return result, err
	}
	...
	if convertErr := ConvertArtemisStatusToBroker(customResource, broker); convertErr != nil {
		reqLogger.Error(convertErr, "failed to convert status back to Broker")
		return result, convertErr
	}

	crStatusUpdateErr := r.UpdateBrokerCRStatus(broker, r.Client, request.NamespacedName)
...
func (r *BrokerReconciler) UpdateBrokerCRStatus(desired *v1.Broker, client rtclient.Client, namespacedName types.NamespacedName) error {
...
func EqualBrokerCRStatus(s1, s2 *v1.BrokerStatus) bool {
...
func brokerExternalConfigsModified(desiredExternalConfigs []v1.ExternalConfigStatus, currentExternalConfigs []v1.ExternalConfigStatus) bool {
