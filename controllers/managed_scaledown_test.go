...
logger.f.name=org.apache.activemq.artemis.core.client.impl
logger.f.level=DEBUG
rootLogger = INFO, STDOUT`
				...
				brokerCrd.Spec.BrokerProperties = []string{
					...
					// scale down to 0 always!
					//"HAPolicyConfiguration.scaleDownConfiguration.connectors=ordinal0",
					//"connectorConfigurations.ordinal0.params.PORT=61616",
					//"connectorConfigurations.ordinal0.params.HOST=" + brokerName + "-ss-0." + brokerName + "-hdls-svc",
					// managed-scaledown51a-ss-0.managed-scaledown51a-hdls-svc.test.svc.cluster.local
					//"connectorConfigurations.ordinal0.factoryClassName=org.apache.activemq.artemis.core.remoting.impl.netty.NettyConnectorFactory",
				...
				Eventually(func(g Gomega) {
					...
					// this is blocked by the need for config reload and restart * 2, I guess it is possible
					// this check is delayed past that, but very unlikely (if this is ever flaky, we can revisit)
					if verbose {
				...
				Eventually(func(g Gomega) {
					...
					// it can take 2x projection reload
		...
		It("deploy plan 2 clustered update rollout", Label("managed-rollout-no-scaledown-check"), func() {
