...
var _ = Describe("artemis as scaledown controller", func() {
	...
	Context("managed scale down test", func() {
		It("deploy plan 3 clustered", Label("managed-scaledown-check"), func() {
			...
			logger.audit_base.level = DEBUG`
				...
				brokerCrd.Spec.DeploymentPlan.Size = common.Int32ToPtr(3)
				...
				brokerCrd.Spec.BrokerProperties = []string{
					...
					// this makes sig term or forceFailover do scaledown and is the trigger for using condition based scaledown
					// 	const ScaleDownConfigTrigger = "HAPolicyConfiguration.scaleDownConfiguration.enabled=true"
		...
		It("deploy plan 2 clustered update rollout", Label("managed-rollout-no-scaledown-check"), func() {
			...
			logger.audit_base.level = DEBUG`
				...
				Eventually(func(g Gomega) {
					...
					By("checking the pod 1 after restart")
					pod0AfterScaleDown := &corev1.Pod{}
					g.Expect(k8sClient.Get(ctx, types.NamespacedName{Name: podWithOrdinal0, Namespace: defaultNamespace}, pod0AfterScaleDown)).Should(Succeed())
