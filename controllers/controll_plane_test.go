...
var _ = Describe("minimal", func() {
	...
	Context("restricted rbac", func() {
		...
		It("control plane override - custom metrics access", func() {
			...
			newUserCertName := "new-user-cert"
			InstallCert(newUserCertName, defaultNamespace, func(candidate *cmv1.Certificate) {
				candidate.Spec.SecretName = newUserCertName
...
`,
			...
			Eventually(func(g Gomega) {
				// Create a copy of http.DefaultTransport to avoid connection reuse between tests
		...
		It("shared control plane override - custom metrics access", func() {
			...
			By("installing new-user cert (should be allowed via shared override)")
			newUserCertName := "new-user-cert"
			InstallCert(newUserCertName, defaultNamespace, func(candidate *cmv1.Certificate) {
				candidate.Spec.SecretName = newUserCertName
				candidate.Spec.CommonName = "new-user"
			...
			By("creating shared control-plane-override secret with custom cert-users and cert-roles")
			sharedOverrideSecretName := "control-plane-override"
			sharedOverrideSecret := corev1.Secret{
				TypeMeta: metav1.TypeMeta{
					Kind:       "Secret",
					APIVersion: "v1",
				},
				ObjectMeta: metav1.ObjectMeta{
					Name:      sharedOverrideSecretName,
					Namespace: defaultNamespace,
				},
				StringData: map[string]string{
					"_cert-users": `# generated by test for shared override
#
hawtio=/CN = hawtio-online\.hawtio\.svc.*/
operator=/.*activemq-artemis-operator.*/
probe=/.*activemq-artemis-operand.*/
new-user=/.*new-user.*/
`,
					"_cert-roles": `# generated by test for shared override
#
status=operator,probe
metrics=operator,new-user
hawtio=hawtio
`,
			...
			Expect(k8sClient.Create(ctx, &sharedOverrideSecret)).Should(Succeed())
			DeferCleanup(func() {
				k8sClient.Delete(ctx, &sharedOverrideSecret)
			})
			...
			Eventually(func(g Gomega) {
				...
				resp, err := httpClient.Get("https://" + serverName + ":8888/metrics")
				fmt.Printf("Test 1 (shared) - Operator cert: status=%d, err=%v\n", resp.StatusCode, err)
				g.Expect(err).Should(Succeed())
				g.Expect(resp.StatusCode).Should(Equal(200))
			...
			By("Test 2: New-user cert can scrape metrics (added via shared override)")
			Eventually(func(g Gomega) {
				// Create a copy of http.DefaultTransport to avoid connection reuse between tests
				transport := http.DefaultTransport.(*http.Transport).Clone()
				httpClient := http.Client{
				...
				resp, err := httpClient.Get("https://" + serverName + ":8888/metrics")
				fmt.Printf("Test 2 (shared) - New-user cert: status=%d, err=%v\n", resp.StatusCode, err)
				g.Expect(err).Should(Succeed())
				g.Expect(resp.StatusCode).Should(Equal(200))
			...
			Eventually(func(g Gomega) {
				...
				resp, err := httpClient.Get("https://" + serverName + ":8888/metrics")
				fmt.Printf("Test 3 (shared) - Random cert: status=%d, err=%v\n", resp.StatusCode, err)
				g.Expect(err).Should(Succeed())
				// Should get 401 Unauthorized or 403 Forbidden
				g.Expect(resp.StatusCode).Should(Or(Equal(401), Equal(403)))
			...
			By("Cleaning up")
			Expect(k8sClient.Delete(ctx, createdCrd)).Should(Succeed())
			Expect(k8sClient.Delete(ctx, &sharedOverrideSecret)).Should(Succeed())
