...
│                                                                              │
│  ┌────────────┐      ┌──────────────────┐      ┌─────────────┐               │
│  │   Client   │─────▶│     CoreDNS      │─────▶│ Prometheus  │               │
│  │ (JMS/AMQP) │      │ emptyendpoints   │      │   metric    │               │
│  └────────────┘      │     plugin       │      └──────┬──────┘               │
│        │             └──────────────────┘             │                      │
│        │                                              ▼                      │
│        │             ┌──────────────────┐      ┌─────────────┐               │
...
kubectl set image deployment/coredns coredns=quay.io/tlavocat/artemis-coredns-plugin:v1.11.1 -n kube-system
...
Run a producer that retries until the broker is available. Each retry triggers a DNS
lookup which increments the CoreDNS metric, causing KEDA to scale up the broker:
...
kubectl run artemis-producer --rm -i --restart=Never \
  ...
  -n serverless-broker -- \
  /bin/bash -c '
    # Disable JVM DNS cache so each retry does a fresh DNS lookup
    ...
    echo "JVM DNS cache disabled - each retry will trigger CoreDNS metric"
    
    for attempt in $(seq 1 20); do
      echo "=== Attempt $attempt at $(date +%H:%M:%S) ==="
      
      timeout 30 /opt/amq/bin/artemis producer \
        --url "amqp://serverless-broker-hdls-svc.serverless-broker.svc.cluster.local:61616" \
        --destination queue://test-queue \
        --message-count 1 \
        --message "Hello from serverless" 2>&1
        
      if [ $? -eq 0 ]; then
        echo "SUCCESS! Message sent to scaled-from-zero broker!"
        exit 0
      fi
