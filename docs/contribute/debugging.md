...
The operator includes Makefile targets to simplify local debugging:
...
```bash
make debug OPERATOR_NAMESPACE=my-namespace
```
...
1. In a terminal, start the Delve server:
   ```bash
   make debug-remote OPERATOR_NAMESPACE=my-namespace
   ```
...
| Target | Description |
|--------|-------------|
| `make debug-setup` | Install CRDs on cluster |
| `make debug` | Start Delve in interactive CLI mode |
| `make debug-remote` | Start Delve on port 2345 for IDE attach |
| `make debug-clean` | Uninstall CRDs from cluster |

### Examples

```bash
# Debug watching all namespaces
make debug OPERATOR_NAMESPACE=activemq-artemis-operator

# Debug watching a single namespace
make debug OPERATOR_NAMESPACE=my-ns WATCH_NAMESPACE=my-ns

# Start remote debugger for IDE
make debug-remote OPERATOR_NAMESPACE=my-ns
```
---
